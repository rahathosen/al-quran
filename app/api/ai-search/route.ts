...
const openai = new OpenAI({
  ...
  defaultHeaders: {
    "HTTP-Referer": process.env.NEXT_PUBLIC_APP_URL || "https://quran-app.vercel.app",
  ...
  try {
    ...
    try {
      ...
      if (Array.isArray(jsonResponse.results)) {
        ...
        return NextResponse.json({ error: "Invalid response format from AI", results: [] }, { status: 500 })
      } else {
        console.error("Unexpected response format:", jsonResponse)
        return NextResponse.json({ error: "Invalid response format from AI", results: [] }, { status: 500 })
      }
      ...
      return NextResponse.json({ error: "Failed to parse AI response", results: [] }, { status: 500 })
    }
  } catch (error) {
