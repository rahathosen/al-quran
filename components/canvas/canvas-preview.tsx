...
export default function CanvasPreview({
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      ctx.fillStyle = colorScheme.background.replace(/[\d.]+\)$/, `${opacity})`)
      ...
      if (translationText) {
        const translationFontSize = translationFontSizeMap[fontSize as keyof typeof translationFontSizeMap]
    ...
    img.onerror = () => {
      ...
      ctx.textAlign = "center"
      ctx.fillText("Error loading image. Please try another background.", canvas.width / 2, canvas.height / 2)
    }
      ...
      shareButton.onclick = async () => {
        try {
          ...
          if (
            navigator.share &&
            navigator.canShare &&
            navigator.canShare({ files: [new File([blob], "quran-verse.jpg", { type: "image/jpeg" })] })
          ) {
            await navigator.share({
              title: "Quran Verse Card",
              text: reference,
              files: [new File([blob], "quran-verse.jpg", { type: "image/jpeg" })],
            })
          } else {
            // Fallback to clipboard copy
            try {
              await navigator.clipboard.writeText("Quran Verse Card - " + reference)
              alert("Image cannot be shared directly. The reference has been copied to your clipboard.")
            } catch (err) {
    ...
    <div className="relative w-full h-full flex items-center justify-center bg-gray-100 min-h-[500px]">
      {!backgroundImage ? (
        <div className="text-center p-8">
          <p className="text-gray-500">Select a background image to preview your verse card</p>
        </div>
      ) : (
      ...
      {isGenerating && (
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
          <div className="bg-white p-4 rounded-lg shadow-lg flex flex-col items-center">
            ...
            <p className="text-[#1a5e63] font-medium">Generating your verse card...</p>
          </div>
        </div>
      )}
    </div>
