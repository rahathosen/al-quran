...
import { Bookmark, BookmarkCheck, PlayCircle, PauseCircle, Loader2, Share2, Copy } from "lucide-react"
import { useAudio } from "@/context/audio-context"
import { useEffect, useRef, useState } from "react"
import { addBookmark, removeBookmark, isVerseBookmarked, type BookmarkedVerse } from "@/lib/local-storage"
...
export default function VerseItem({
  ...
  const { isPlaying, isLoading, currentVerse, currentSurah, playVerse, pauseAudio } = useAudio()

  const [bookmarked, setBookmarked] = useState(false)
  const verseRef = useRef<HTMLDivElement>(null)

  const isCurrentVerse = currentVerse === verse.number && currentSurah === surahId
  ...
  const verseNumberSize =
    viewMode === "arabic-only" ? "w-6 h-6 text-sm" : viewMode === "compact" ? "w-7 h-7 text-sm" : "w-8 h-8 text-base"

  // Determine Arabic text size based on view mode
  const arabicTextSize = viewMode === "arabic-only" ? "text-xl" : viewMode === "compact" ? "text-xl" : "text-2xl"

  return (
    <div
      ref={verseRef}
      className={`${borderClass} transition-all duration-300 ${
        isCurrentVerse ? "bg-[#f8f5f0] rounded-lg p-4 -mx-4 shadow-md sticky top-14 z-10" : ""
      }`}
      id={`verse-${verse.number}`}
    >
      <div
        className={`flex justify-between items-center ${viewMode === "arabic-only" ? "mb-1" : viewMode === "compact" ? "mb-1.5" : "mb-2"}`}
      >
        <div className="flex items-center gap-2">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="ghost" size="icon" className="h-8 w-8 text-[#1a5e63]" onClick={toggleBookmark}>
              {bookmarked ? <BookmarkCheck className="h-4 w-4 fill-[#1a5e63]" /> : <Bookmark className="h-4 w-4" />}
            </Button>
            <Button variant="ghost" size="icon" className="h-8 w-8 text-[#1a5e63]" onClick={handleShareVerse}>
              <Share2 className="h-4 w-4" />
            </Button>
            <Button variant="ghost" size="icon" className="h-8 w-8 text-[#1a5e63]" onClick={handleCopyVerse}>
      ...
      <p
        ...
        } quran-${quranFont} ${isCurrentVerse ? "text-[#1a5e63]" : "text-[#333]"} relative`}
      >
        <span
      ...
      {(viewMode === "default" || viewMode === "compact") && (
        <p className={`verse-translation ${viewMode === "compact" ? "text-sm" : "text-base"} text-[#555]`}>
          {verse.translation}
        </p>
      )}
    </div>
