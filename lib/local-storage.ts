...
const STORAGE_KEYS = {
  ...
  SURAH_BOOKMARKS: "quran-surah-bookmarks",
...
export function addBookmark(bookmark: BookmarkedVerse): void {
  ...
  try {
    ...
    const existingIndex = bookmarks.findIndex(
      (b) =>
        b.surahId === bookmark.surahId && b.verseNumber === bookmark.verseNumber
    );
...
export function getBookmarkedSurahs(): BookmarkedSurah[] {
  if (typeof window === "undefined") return [];

  try {
    const bookmarks = localStorage.getItem(STORAGE_KEYS.SURAH_BOOKMARKS);
    return bookmarks ? JSON.parse(bookmarks) : [];
  } catch (error) {
...
export function addSurahBookmark(bookmark: BookmarkedSurah): void {
  if (typeof window === "undefined") return;

  try {
    const bookmarks = getBookmarkedSurahs();

    // Check if this surah is already bookmarked
    const existingIndex = bookmarks.findIndex(
      (b) => b.surahId === bookmark.surahId
    );
...
export function removeSurahBookmark(surahId: number): void {
  if (typeof window === "undefined") return;

  try {
    const bookmarks = getBookmarkedSurahs();
    const filteredBookmarks = bookmarks.filter((b) => b.surahId !== surahId);
...
export function isSurahBookmarked(surahId: number): boolean {
  if (typeof window === "undefined") return false;

  try {
    const bookmarks = getBookmarkedSurahs();
    return bookmarks.some((b) => b.surahId === surahId);
  } catch (error) {
...
export function updateRecentSurah(surah: RecentSurah): void {
  ...
  try {
    ...
    const existingIndex = recentSurahs.findIndex(
      (s) => s.surahId === surah.surahId
    );
    ...
    const sortedSurahs = recentSurahs
      .sort((a, b) => b.lastRead - a.lastRead)
      .slice(0, 5);

    localStorage.setItem(
      STORAGE_KEYS.RECENT_SURAHS,
      JSON.stringify(sortedSurahs)
    );
  } catch (error) {
